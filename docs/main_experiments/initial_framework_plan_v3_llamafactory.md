...
- GPT-4o judge wrapper (LF has no safety eval)
- Safety-specific metrics (ASR/RS/HR/FPR + PD/PC/VS)
- Paper tables
...
- E4 V2/V4 general-data mix: held open. **Plan must remind user before V2/V4 training kicks off** (memory rule recorded).
...
**Smoke test (Phase 1)**:
```bash
conda run -n mis_safety python -c "
import asyncio
...
conda run -n mis_safety python scripts/build_cf_pairs.py \
  ...
  --benign-source openimages --benign-pool-size 5000 \
  --output /mnt/hdd/xuran/mis_dataset_builder/dataset/test_cf.json \
  --quality-judge gpt-4o-mini --max-retries 3
```
...
conda run -n mis_safety python scripts/build_cf_pairs.py \
  --test-json /mnt/hdd/xuran/mis_dataset_builder/dataset/test.json \
  --benign-source openimages --benign-pool-size 100 \
  --output /tmp/test_cf_smoke.json --limit 50 --skip-judge
```
...
6. ✅ **E4 V2/V4 general data mix**: held open; runtime reminder fires before launch (see Active reminders).
...
|---------|----------|-------|
| Before E4 V2 / V4 training run | Surface to user: "general-data mix for V2/V4 is held open. Decide ratio + source (e.g., ShareGPT4V N samples : DREAMS N samples) before proceeding." | `scripts/run_capability.py` checks V2/V4 config has `enabled: true` + `general_data` block populated; raises `RuntimeError` with reminder otherwise |
