...
│       │   ├── E4_V2_internvl3_5_mirage_general.yaml  # BLOCKED — 待填 general_data
│       │   ├── E4_V3_internvl3_5_dreams.yaml        # E4 V3（DREAMS 数据）
│       │   ├── E4_V4_internvl3_5_dreams_general.yaml  # BLOCKED — 待填 general_data
│       │   ├── _cohort.yaml                         # 模型队列定义
...
│   │   ├── cf_synthesizer.py          # E5 反事实样本合成（image-swap）
...
│   └── reporting/
│       ├── table_generator.py        # 论文表格生成（E1–E5 + 消融 + prelim）
│       └── aggregator.py             # 结果聚合
│
├── scripts/                          # CLI 入口
│   ├── run_experiment.py             # 单实验端到端（训练 → 推理 → 评测）
│   ├── run_main.py                   # E1/E2/E3/E5 队列编排器
...
│   ├── build_cf_pairs.py             # 离线构建 E5 反事实对
...
### 4. 评测指标（`harness/evaluation/metrics.py`）

**安全指标（MIS 协议，ASR + RSR + RR + HR = 1.0）**：
- **ASR** (↓) = count(Unsafe) / total — 攻击成功率
- **RSR** (↑) = count(Safe & Perception Level1) / total — 感知正确后安全拒绝
- **RR**  (↓) = count(Safe & Perception Level2) / total — 未感知图像内容的盲拒绝
- **HR**  (↓) = count(Safe & Perception Level3) / total — 看不见图像而拒绝（最差情形）
...
# Step 2: 构建 E5 反事实对（需要 benign image 目录）
...
conda run -n mis_safety python scripts/run_experiment.py \
    main/main_dreams_internvl3_5.yaml --dry-run --limit 5
...
conda run -n mis_safety python scripts/run_experiment.py \
    main/main_dreams_internvl3_5.yaml \
    --skip-train --model-path /mnt/hdd/xuran/vlm_safety_harness/models/internvl3_5_dreams/
```
...
```bash
# V0/V1/V3（V2/V4 blocked，需先填写 general_data 配置块）
conda run -n mis_safety python scripts/run_capability.py \
    --variants V0 V1 V3 \
    --baseline internvl3_5 \
    --benchmarks mmstar mmmu muirbench blink mmt

# V2/V4 运行时会报 BLOCKED 提示，要求在配置中指定：
#   training.general_data.sources / ratio / total_samples
```

### E5 — 反事实一致性（需先生成 test_cf.json）
...
conda run -n mis_safety python scripts/run_closed_source.py \
    ...
    --benchmarks our_test mis_easy mis_hard \
    --limit 100 --dry-run  # 先 dry-run
...
```bash
# 仅推理
conda run -n mis_safety python scripts/run_inference_only.py \
    --config main/main_dreams_internvl3_5.yaml \
...
conda run -n mis_safety python scripts/run_eval_only.py \
    --responses results/main/main_dreams_internvl3_5/YYYYMMDD_HHMMSS/responses/ \
    --evaluator-type auto

# 规则评测（AdvBench 关键词匹配）
conda run -n mis_safety python scripts/run_eval_only.py \
    --responses results/main/.../responses/ \
    --benchmarks advbench --evaluator-type rule

# 准确率评测（MMStar 多选题）
conda run -n mis_safety python scripts/run_eval_only.py \
    --responses results/main/.../responses/ \
    --benchmarks mmstar --evaluator-type accuracy
```
...
conda run -n mis_safety python scripts/generate_report.py \
    --group main --format latex markdown

# 单个实验
conda run -n mis_safety python scripts/generate_report.py \
    --group main --experiment-set e1 --format markdown
```
...
| E4 V2/V4 | 运行前必须在配置中填写 `training.general_data`（sources / ratio / total_samples） |
| E5 | 运行前必须先生成 `test_cf.json`（`build_cf_pairs.py`） |
...
| E5 CF pairs | `/mnt/hdd/xuran/mis_dataset_builder/dataset/test_cf.json`（生成后） |
